...
/**
 * Converts between geodetic coordinates (latitude, longitude, ellipsoidal height)
 * and body-centered, body-fixed Cartesian coordinates, for a given reference ellipsoid.
 * <p>
 * The ellipsoid is described by its equatorial radius  a
 * and the square of its first eccentricity  e^2 .
...
public class GeodeticCoordinatesConverter
{
	 ...
	 * <p>
	 * Unlike {@link #toCartesian(GeodeticCoordinates)}, this conversion is not exact:
	 * Bowring's method is a very accurate closed-form approximation
	 * (sub-millimeter for near-surface points), not an exact inversion.
	 *
	 * @param position	Cartesian position in the body-centered, body-fixed frame. [m]
	 * @return	geodetic coordinates of the position.
	 */
	public GeodeticCoordinates fromCartesian( Vector3 position )
	{
		...
		double height = p / Math.cos( phi ) - this.primeVerticalRadiusOfCurvature( phi );
		GeodeticCoordinates output = new GeodeticCoordinates();
	 ...
	 */
	public Vector3 toCartesian( GeodeticCoordinates coordinates )
	{
